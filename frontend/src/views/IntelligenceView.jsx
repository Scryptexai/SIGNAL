...
export default function IntelligenceView({ target }) {
    ...
    (async () => {
      try {
        if (target.type === "entity") {
          const data = await api.getEntity(target.value);
          if (!active) return;
          setProfile(data.entity);
          setBalances(data.balances);
          setCps(data.counterparties);
        } else {
          const data = await api.getAddress(target.value);
          if (!active) return;
  ...
  const entityType = isEntity ? profile?.type : profile?.arkhamEntity?.type;
  const maxChain = chains.length ? chains[0].usd : 1;

  const aiData = useMemo(
    () => ({
      entity: { name: title, type: entityType },
      balances: {
        total_usd: portfolio,
        top_tokens: tokens.slice(0, 6).map((t) => ({ symbol: t.symbol, usd: t.usd })),
      },
      counterparties: cpsRows.slice(0, 6).map((c) => ({
        name: partyName(c.address),
        usd: c.usd,
      })),
    }),
    [title, entityType, portfolio, tokens, cpsRows]
  );
    ...
    <div className="fade-in p-6">
      ...
      >
        <div className="min-w-0">
          ...
          {isEntity && (
            <div className="mt-3 flex flex-wrap gap-3">
              {profile?.website && (
                <a href={profile.website} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-dim hover:text-primary">
                  <Globe size={13} /> Website <ArrowSquareOut size={11} />
                </a>
              )}
              {profile?.twitter && (
                <a href={profile.twitter} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-dim hover:text-primary">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          <Stat label="24h Change" value={`${delta >= 0 ? "+" : ""}${usd(delta)}`} accent={delta >= 0 ? "text-pos" : "text-neg"} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                {chains.map((c) => (
                  <div key={c.chain}>
                    ...
                    <div className="h-1.5 w-full bg-bg">
                      <div className="h-full bg-primary" style={{ width: `${Math.max(2, (c.usd / maxChain) * 100)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="lg:col-span-4">
          <AIPanel dataType="entity_profile" rawData={aiData} subject={title} disabled={loading} />
        </div>

        {isEntity && (
          <div className="lg:col-span-12">
            <Panel title="Top Counterparties" sub="Highest-volume interacting addresses (7d)">
              {!cps ? (
                    ...
                    {cpsRows.map((c, i) => (
                      <tr key={i} className="border-t border-border/60 hover:bg-surfaceHover">
                        <td className="py-1.5">
                          ...
                          <span className="ml-2 font-mono text-[11px] text-dim">{shortAddr(c.address?.address)}</span>
                        </td>
